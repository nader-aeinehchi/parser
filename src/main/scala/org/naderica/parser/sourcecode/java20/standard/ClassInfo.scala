...
/** A simple data structure to hold a class's classInfo and its methods. */
class ClassInfo {
  ...
  val innerClasses: ListBuffer[ClassInfo] = ListBuffer.empty
  ...
  /** Produce a JSON representation of the ClassInfo.
    *
  ...
  def toJson(): String = {
    ...
    val packageSignatureJson = s""""package": "$packageSignature""""
